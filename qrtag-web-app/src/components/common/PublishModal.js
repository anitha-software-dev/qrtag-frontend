...
export default function BasicModal({ open, setOpen, itemId, getList }) {
  ...
  const handlePublish = () => {
    ...
    setLoading(true);
    try {
      Axios.patch(`/common/digital-memorial/${itemId}/`, params)
        .then((response) => {
          setLoading(false);
          if (response && response.status === 200) {
            setOpen(false)
            toast.success('Digital Memorial published successfully!');
            getList()
          } else {
