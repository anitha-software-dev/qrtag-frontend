...
function DeleteAccountCode() {
  ...
  useEffect(() => {
    if (messages) {
      ...
      const res = messages.filter((item) => item.key === 'didnot_receive_token');
      if (res.length > 0) {
        setDidntReceiveToken(res[0]);
      }
  ...
  async function resendConfirmationCode() {
    ...
    try {
      AuthService('/users/delete-user/resend-code/', { email: username }, (response) => {
        setLoading(false);
  ...
  async function confirmDelete() {
    ...
    setLoading(true);
    try {
      AuthService('/users/delete-user/verify-code/', { email: username, code: formState.code }, (response) => {
        setLoading(false);
        if (response && response.success) {
          nav('/')
        ...
        <div className='LogIn_RightDiv Auth_Bg'
          ...
          <form className='LogIn_FormDiv'
            ...
            <div className='LogIn_HeadingDiv' >
              ...
              <p >
                To continue please enter the 6 digit token sent to your email</p>
            </div>
            <div
              style={{
                display: 'flex',
            ...
            <div className='LogIn_BtnDiv'
              ...
              <button onClick={confirmDelete} className='LogIn_Btn' style={{ height: '55px' }}>
