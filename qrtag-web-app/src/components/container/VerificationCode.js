...
import { AuthResendSignup, AuthConfirmSignup } from '../../services/auth';
...
function VerificationCode() {
  ...
  useEffect(() => {
    if (messages) {
      ...
      const res = messages.filter((item) => item.key === 'verification_heading_message');
      if (res.length > 0) {
  ...
  async function confirmSignUp() {
    ...
    setLoading((s) => !s);
    try {
      AuthConfirmSignup({ email: username, code: formState.code }, (response) => {
        setLoading((s) => !s);
        if (response && response.success) {
          nav('/acceptterms', { state: { username: username } })
  ...
  return (
    <>
      ...
      <div className='LogInContainer'>
        ...
        <div className='LogIn_RightDiv Auth_Bg' style={{
          display: 'flex',
          ...
          <form className='LogIn_FormDiv'
            ...
            <div className='LogIn_HeadingDiv'>
              ...
              <p>{(headingContent && headingContent?.value) ? headingContent?.value : "To continue please enter the 6 digit token sent to your email"}</p>
            </div>
            <div
              style={{
                display: 'flex',
            ...
            <div className='LogIn_BtnDiv' style={{
              marginTop: '5%'
            }}>
              <button onClick={confirmSignUp} className='LogIn_Btn' style={{ height: '55px' }}>
