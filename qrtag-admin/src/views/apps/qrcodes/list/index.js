...
const QRList = () => {
  ...
  const getQRList = async (param = {}) => {
    ...
    try {
      ...
      const searchKeyword = (param && param.reset) ? '' : (param.search || search)
      const page = (param && param.reset) ? 1 : (param.page || currentPage)
      const limitCount = (param && param.reset) ? 10 : (param.limit || rowsPerPage)
      const usedFilter = (param && param.reset) ? '' : (param.used !== undefined ? param.used : used)

      let baseQuery = query(collectionRef)
      if (usedFilter !== '') {
        baseQuery = query(baseQuery, where('used', '==', usedFilter))
      }
      ...
      if (searchKeyword.length >= 3) {
        results = docs.filter(doc => {
          const uuidMatch = doc.uuid && doc.uuid.toLowerCase().includes(searchKeyword.toLowerCase())
          const emailMatch = doc.user?.email && doc.user.email.toLowerCase().includes(searchKeyword.toLowerCase())
          const nameMatch = doc.user?.name && doc.user.name.toLowerCase().includes(searchKeyword.toLowerCase())
      ...
      const paginatedResults = results.slice(startIndex, startIndex + limitCount)

      setQRList(paginatedResults)
      setTotalRecords(results.length)

    } catch (error) {
      console.error('Error fetching QR tags:', error)
  ...
  const handlePagination = page => {
    ...
    getQRList({ search, page: page.selected + 1 })
  }
  ...
  const columns = [
      ...
      cell: row => <>
        {(row && row.user) ? <>
          <div className='d-flex flex-column'>
            <span className='text-primary font-weight-bold cursor-pointer' onClick={() => { setEditInfo(row); setUserModal(!userModal); getUserData(row.user.email) }}>{(row.user.name) ? row.user.name : '-'}</span>
    ...
    },
    {
      ...
      selector: 'status',
      sortable: false,
      cell: row => <>{(row.item && row.item.status && row.item.status === 'lost') ? <Badge pill color='danger'>LOST</Badge> : (row.item && row.item.status && row.item.status === 'found') ? <Badge pill color='primary'>FOUND</Badge> : '-'}</>
    },
    {
      name: 'Action',
      center: true,
  ...
  return (
    <>
            ...
            <Col md="7" className="d-flex align-items-center justify-content-center">
              <div className='mr-1 w-100'>
                <InputGroup>
                  ...
                  <InputGroupAddon addonType='append'>
                    <Button onClick={() => {
                      if (search !== '') {
                        getQRList({ search, page: 1 }); setCurrentPage(1)
                      }
              ...
              <CustomInput
                ...
                onChange={(e) => {
                  ...
                  setCurrentPage(1)
                  getQRList({ used: e.target.value, page: 1 })
                }}
              >
                ...
                <Row>
                  ...
                  <Col className="mb-2" md='7'>
                    <Row>
                      ...
                      <Col md='4' className="mb-2"><strong>STATUS</strong> <dd>{(editInfo && editInfo.item && editInfo.item.status && editInfo.item.status === 'lost') ? <Badge pill color='danger'>LOST</Badge> : (editInfo && editInfo.item && editInfo.item.status && editInfo.item.status === 'found') ? <Badge pill color='primary'>FOUND</Badge> : '-'}</dd></Col>
                      <Col md='8' className="mb-2"><strong>ITEM</strong> <dd>{(editInfo && editInfo.item && editInfo.item.name) ? editInfo.item.name : '-'}</dd></Col>
                    </Row>
                  </Col>

                  <Col md='12'><strong>CODE</strong> <dd>{editInfo && editInfo.uuid ? editInfo.uuid : '-'}</dd></Col>
      ...
      {/* View User Details */}
      <UserModal setUserModal={setUserModal} userModal={userModal} userData={userData} fetching={fetching} editInfo={editInfo} />
